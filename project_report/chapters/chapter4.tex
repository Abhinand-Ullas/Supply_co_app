...
\begin{figure}[H]
\centering
\includegraphics[height=1.2\textheight,width=1.2\textwidth,keepaspectratio]{images/er_daigram.png}
\end{figure}
...
\textbf{API Endpoint:}
The simulator exposes a single REST endpoint that accepts a \newline \texttt{last\_sync\_time} query parameter and returns all stock records modified after that timestamp. This enables incremental synchronization, where only changed records are transferred during each sync cycle.

...
\begin{itemize}
    \item \textbf{Unit Testing:} Individual service methods were tested in isolation. The \newline \texttt{SupabaseService} data transformation methods correctly flattened the joined stock-items response into the expected format across all test cases. The \newline \texttt{StorageService} methods for cache serialization, language code mapping, and time-ago calculation produced correct results for all edge cases (including empty cache, expired cache, and boundary time values).
...
\hline
Unit Tests & Service methods, data transformation, cache operations & 24 & 95\% \\
\hline
Integration Tests & Auth flow, data sync, push notifications, API calls & 12 & 95\% \\
\hline
...
\hline
Offline Tests & Cache display, offline banner, retry mechanism & 6 & 95\% \\
\hline
\textbf{Total} & & \textbf{68} & \textbf{93.5\%} \\
\hline
\end{tabularx}
\end{table}

The 6.5\% failure rate was attributable to minor UI rendering issues on devices with non-standard screen densities, GPS permission flow inconsistencies on specific Android OEM implementations (e.g., Xiaomi MIUI), and occasional edge cases in cache operations and data synchronization. These issues were identified as device-specific or environment-specific rather than application-level defects.
